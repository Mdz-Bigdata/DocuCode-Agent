...
async def websocket_chat(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            # 优化：小智客服优先使用快速发送（模拟响应），不逐字符延迟
            if agent_type == 'customer_service' and len(response_text) < 500:
                # 对于客服响应，直接发送完整内容，速度最快
                await websocket.send_json({"type": "stream", "content": response_text})
            else:
                # 对于其他情况，稍微延迟但比原来快
                chunk_size = 5  # 一次发送 5 个字符，更流畅
                for i in range(0, len(response_text), chunk_size):
                    chunk = response_text[i:i + chunk_size]
                    await websocket.send_json({"type": "stream", "content": chunk})
                    await asyncio.sleep(0.005)  # 从 0.02 秒减少到 0.005 秒
